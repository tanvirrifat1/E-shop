...
const AllUsers = () => {
    ...
    const { data: users = [], refetch } = useQuery({
        queryKey: ['users'],
        queryFn: async () => {
            const res = await fetch('https://e-shop-server-tanvirrifat1.vercel.app/users')
    ...
    const handleMakeAdmin = id => {
        fetch(`https://e-shop-server-tanvirrifat1.vercel.app/users/admin/${id}`, {
            method: 'PUT',
            headers: {
    ...
    const handleDelete = id => {
        fetch(`https://e-shop-server-tanvirrifat1.vercel.app/users/${id}`, {
            method: 'DELETE',
            headers: {
                authorization: `bearer ${localStorage.getItem('accessToken')}`
            }
    ...
    return (
        <div className='ml-2'>
            <h2 className='text-3xl text-center my-3 font-semibold'>All Users</h2>
